...
{
    internal class DarkRiftFragment : ModItem
    {
        ...
        {
            Lighting.AddLight(Item.Center, Color.Purple.ToVector3());
        }
        public override bool PreDrawInWorld(SpriteBatch spriteBatch, Color lightColor, Color alphaColor, ref float rotation, ref float scale, int whoAmI)
        {
            UpdateScalingFactor();
            Color color = new Color(93, 18, 186);
            ...
            spriteBatch.Draw(glowTexture,
                ...
                color, rotation, glowTexture.Size() * 0.5f, (scale) * scalingFactor, SpriteEffects.None, 0f);

            spriteBatch.Draw(glowTexture,
                new Vector2(Item.position.X - Main.screenPosition.X + Item.width * 0.5f, Item.position.Y - Main.screenPosition.Y + Item.height * 0.5f),
                new Rectangle(0, 0, glowTexture.Width, glowTexture.Height),
                Color.White, rotation, glowTexture.Size() * 0.5f, (scale * 0.5f) * scalingFactor, SpriteEffects.None, 0f);

            //draw texture
            Asset<Texture2D> itemTexture = ModContent.Request<Texture2D>("TheTesseractMod/Items/Materials/DarkRiftFragment");
            spriteBatch.Draw(itemTexture.Value,
                new Vector2(Item.position.X - Main.screenPosition.X + Item.width * 0.5f, Item.position.Y - Main.screenPosition.Y + Item.height * 0.5f),
                new Rectangle(0, 0, itemTexture.Value.Width, itemTexture.Value.Height),
                Color.White, rotation, itemTexture.Value.Size() * 0.5f, (scale * 1.7f) * scalingFactor, SpriteEffects.None, 0f);
            return false;
        }
        public override bool PreDrawInInventory(SpriteBatch spriteBatch, Vector2 position, Rectangle frame, Color drawColor, Color itemColor, Vector2 origin, float scale)
        {
            UpdateScalingFactor();
            Asset<Texture2D> texture = ModContent.Request<Texture2D>("TheTesseractMod/Items/Materials/DarkRiftFragment");
            spriteBatch.Draw(texture.Value, position, null,
               Color.White, 0, texture.Size() / 2, ((scale * 2)) * scalingFactor, SpriteEffects.None, 0f);
