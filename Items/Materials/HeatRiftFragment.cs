...
{
    internal class HeatRiftFragment : ModItem
    {
        ...
        {
            Lighting.AddLight(Item.Center, Color.OrangeRed.ToVector3());
            if (dustTimer%12 == 0)
            {
                Dust.NewDust(Item.position, 10, 10, DustID.Torch);
            }
           dustTimer++;
        }
        ...
        public override bool PreDrawInWorld(SpriteBatch spriteBatch, Color lightColor, Color alphaColor, ref float rotation, ref float scale, int whoAmI)
        {
            ...
            spriteBatch.Draw(glowTexture,
                ...
                Color.OrangeRed, rotation, glowTexture.Size() * 0.5f, (scale * 0.5f) * scalingFactor, SpriteEffects.None, 0f);

            UpdateScalingFactor();
            //draw texture
            Asset<Texture2D> itemTexture = ModContent.Request<Texture2D>("TheTesseractMod/Items/Materials/HeatRiftFragment");
            spriteBatch.Draw(itemTexture.Value,
                new Vector2(Item.position.X - Main.screenPosition.X + Item.width * 0.5f, Item.position.Y - Main.screenPosition.Y + Item.height * 0.5f),
                new Rectangle(0, 0, itemTexture.Value.Width, itemTexture.Value.Height),
                Color.White, rotation, itemTexture.Value.Size() * 0.5f, (scale * 2f) * scalingFactor, SpriteEffects.None, 0f);
        ...
        {
            UpdateScalingFactor();
            Asset<Texture2D> texture = ModContent.Request<Texture2D>("TheTesseractMod/Items/Materials/HeatRiftFragment");
