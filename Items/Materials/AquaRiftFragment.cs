...
{
    internal class AquaRiftFragment : ModItem
    {
        ...
        {
            Lighting.AddLight(Item.Center, Color.Aqua.ToVector3());
        }
        public override bool PreDrawInWorld(SpriteBatch spriteBatch, Color lightColor, Color alphaColor, ref float rotation, ref float scale, int whoAmI)
        {
            ...
            spriteBatch.Draw(glowTexture,
                ...
                Color.Aqua, rotation, glowTexture.Size() * 0.5f, (scale * 0.5f) * scalingFactor, SpriteEffects.None, 0f);

            UpdateScalingFactor();
            //draw texture
            Asset<Texture2D> itemTexture = ModContent.Request<Texture2D>("TheTesseractMod/Items/Materials/AquaRiftFragment");
            spriteBatch.Draw(itemTexture.Value,
                new Vector2(Item.position.X - Main.screenPosition.X + Item.width * 0.5f, Item.position.Y - Main.screenPosition.Y + Item.height * 0.5f),
                new Rectangle(0, 0, itemTexture.Value.Width, itemTexture.Value.Height),
                Color.White, rotation, itemTexture.Value.Size() * 0.5f, (scale * 1.3f) * scalingFactor, SpriteEffects.None, 0f);
            return false;
        }
        ...
        {
            UpdateScalingFactor();
            Asset<Texture2D> texture = ModContent.Request<Texture2D>("TheTesseractMod/Items/Materials/AquaRiftFragment");
            spriteBatch.Draw(texture.Value, position, null,
               Color.White, 0, texture.Size() / 2, (scale * 1.3f) * scalingFactor, SpriteEffects.None, 0f);
