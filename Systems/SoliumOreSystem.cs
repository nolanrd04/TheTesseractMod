...
{
	public class SoliumOreSystem : ModSystem
	{
		public void BlessWorldWithSoliumOre() {
			if (Main.netMode == NetmodeID.MultiplayerClient) {
				return;
			}

			int tileType = ModContent.TileType<SoliumOreTile>();
			...
			if (Main.netMode == NetmodeID.SinglePlayer) {
				Main.NewText("Your world has been blessed with Solium Ore!", 100, 100, 220);
			}
			else if (Main.netMode == NetmodeID.Server) {
				Terraria.Chat.ChatHelper.BroadcastChatMessage(Terraria.Localization.NetworkText.FromLiteral("Your world has been blessed with Solium Ore!"), new Microsoft.Xna.Framework.Color(100, 100, 220));
			}
		}
	}
}
