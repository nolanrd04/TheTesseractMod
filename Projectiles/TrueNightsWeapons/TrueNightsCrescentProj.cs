...
{
    internal class TrueNightsCrescentProj : ModProjectile
    {
        ...
        float speed = 10f;
        int attackStage = 0;        // the type of attack this minion will use. 0 is ricochete, 1 is dash.
        ...
        private bool CheckActive(Player owner)
        {
            ...
            {
                owner.ClearBuff(ModContent.BuffType<TrueCrescentMinionBuff>());

                return false;
            }

            if (owner.HasBuff(ModContent.BuffType<TrueCrescentMinionBuff>()))
            {
        ...
        {
            NPC target;
            if (attackStage == 0) // during richochete stage, the target will be manually set instead of set by the player's whip
            {
                target = GlobalProjectileFunctions.findClosestTargetInRange(Projectile.Center, lastHit, attackSight);
            }
            else
            {
            ...
            if (foundTarget)
            {
                ...
                if (attackStage == 0) // ricochete stage
                {
                    speed = 20f;
                    inertia = 30f;
        ...
        private void Visuals()
        {
            if (attackStage == 0)
            {
                ...
                {
                    Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, ModContent.DustType<SharpRadialGlowDust>(), 0, 0, 0, Color.Lime, .5f);
            ...
            else
            {
                ...
                if (Projectile.ai[0] % 2 == 0)
                {
                    for (int i = 0; i < 3; i++)
                    {
                        Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, ModContent.DustType<SharpRadialGlowDust>(), 0, 0, 0, Color.DeepPink, .5f);
