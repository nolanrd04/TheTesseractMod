...
{
    internal class Shin_Kal_Proj_Stage1 : ModProjectile
    {
        ...
        public override void AI()
        {
            if (canHome)
            {
                ...
                if (Math.Abs(Vector2.Distance(Main.MouseWorld, Projectile.Center)) > 30f)
                {
                    ...
                    Projectile.velocity = Vector2.Lerp(Projectile.velocity, velocity, .07f);
                }
                else
                {
            ...
            {
                Dust.NewDust(Projectile.Center, 0, 0, ModContent.DustType<SharpRadialGlowDust>(), 0, 0, 0, new Color(238, 54, 255, 0), Main.rand.NextFloat(.5f) + .6f);
            }
            ...
            {
                Dust.NewDust(Projectile.Center, 0, 0, ModContent.DustType<SharpRadialGlowDust>(), 0, 0, 0, new Color(242, 218, 58, 0), Main.rand.NextFloat(.3f) + .4f);
            }
        }
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            for (int i = 0; i < numDivisions; i++)
            {
                ...
                {
                    Dust.NewDust(Projectile.Center, 0, 0, ModContent.DustType<SharpRadialGlowDust>(), velocity.X, velocity.Y, 0, new Color(238, 54, 255, 0), Main.rand.NextFloat(.4f) + .4f);
                }
                else
                {
                    Dust.NewDust(Projectile.Center, 0, 0, DustID.PinkTorch, velocity.X, velocity.Y, 0, default(Color), Main.rand.NextFloat(.8f) + .4f);
                }
            }
