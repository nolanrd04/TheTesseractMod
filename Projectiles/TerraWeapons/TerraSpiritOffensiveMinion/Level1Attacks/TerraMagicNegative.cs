...
namespace TheTesseractMod.Projectiles.TerraWeapons.TerraSpiritOffensiveMinion.Level1Attacks
{
    internal class TerraMagicNegative : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            Projectile.position.Y -= (float)(Math.Sin(incrementalAngle) * Math.Cos(-direction) * amplitude);
            Projectile.position.X -= (float)(Math.Sin(incrementalAngle) * Math.Sin(-direction) * amplitude);
            Projectile.ai[0]++;
        }
    }
}
