...
{
	public class WhipOfTheWestProj : ModProjectile
	{
		// The texture doesn't have the same name as the item, so this property points to it.
		public override string Texture => "TheTesseractMod/Projectiles/Summoner/WhipOfTheWestProj";
		...
		public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
			target.AddBuff(ModContent.BuffType<WhipOfTheWestTagBuff>(), 240);
			...
			// spawn rocks to throw at the target

			int npcWidth = target.width;
			int numRocks = Main.rand.Next(2, 4);
			for (int i = 0; i < numRocks; i++)
			{
				Vector2 spawnPos = target.Center + new Vector2(npcWidth + Main.rand.Next(35, 45), 0f).RotatedByRandom(MathHelper.TwoPi);
				Vector2 direction = target.Center - spawnPos;
				direction.Normalize();
				float rotationalAngle = 0f;
				if (spawnPos.X > target.Center.X)
				{
					rotationalAngle = MathHelper.ToRadians(-135f);
				}
				else
				{
					rotationalAngle = MathHelper.ToRadians(-45f);
				}
				direction = Vector2.UnitX.RotatedBy(rotationalAngle) * 10f;
				if (Main.netMode != NetmodeID.MultiplayerClient)
				{
					Projectile.NewProjectile(Projectile.InheritSource(Projectile), spawnPos, direction, ModContent.ProjectileType<WhipOfTheWestRock>(), Projectile.damage / 2, 0f, Projectile.owner);
				}
			}
		}
		...
		private void DrawLine(List<Vector2> list) {
			Texture2D texture = ModContent.Request<Texture2D>("TheTesseractMod/Textures/WhipOfTheWestConnector").Value;
			...
			for (int i = 0; i < list.Count - 1; i++) {
				...
				if (i > 0) {  // Only draw lines after the handle
		...
		public override bool PreDraw(ref Color lightColor) {
			...
			for (int i = 0; i < list.Count - 1; i++) {
				Rectangle frame = new Rectangle(14, 0, 10, 26); // Handle at X=14
				Vector2 origin = new Vector2(5, 13); // Handle
				float scale = 1;

				if (i == list.Count - 2) {
					// Tip (skull) - larger frame
					frame = new Rectangle(0, 66, 38, 44);
					origin = new Vector2(19, 22); // Center the larger skull sprite
					...
					// Random segment (1, 2, or 3) - all at X=14
					int segmentChoice = Main.rand.Next(3);
					if (segmentChoice == 0) {
						frame = new Rectangle(14, 28, 10, 10); // Segment 1
						origin = new Vector2(5, 5);
					}
					else if (segmentChoice == 1) {
						frame = new Rectangle(14, 40, 10, 10); // Segment 2
						origin = new Vector2(5, 5);
					}
					else {
						frame = new Rectangle(14, 52, 10, 12); // Segment 3
				...
				float rotation = diff.ToRotation() - MathHelper.PiOver2;
				Color color = Color.White; // TEMP: Force white to rule out lighting issues
